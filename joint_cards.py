...
class JointCards:
    def __init__(self, bbox_info):
        """
        :param bbox_info:  图片中所有bbox的坐标, [["class", x1, y1, x2, y2, x3, y3, x4, y4],[],...[]]
        """
        self.__bbox_info = bbox_info
        self.__bbox_locations = []
        self.__bbox_chars = []
        self.__preprocess()
        
    def __preprocess(self,):
        location_list = []
        for i in range(len(self.__bbox_info)):
            char = str(self.__bbox_info[i][0])
            x1 = float(self.__bbox_info[i][1])
            y1 = float(self.__bbox_info[i][2])
            x2 = float(self.__bbox_info[i][3])
            y2 = float(self.__bbox_info[i][4])
            x3 = float(self.__bbox_info[i][5])
            y3 = float(self.__bbox_info[i][6])
            x4 = float(self.__bbox_info[i][7])
            y4 = float(self.__bbox_info[i][8])
            xs = [x1, x2, x3, x4]
            ys = [y1, y2, y3, y4]
            # 计算正外接矩形
            xmin, ymin, xmax, ymax = self.__min_rectangle(xs, ys)
            # 从左上开始，顺时针记录四个点, 分别为p1->p2->p3->p4
            self.__bbox_locations.append([xmin, ymin, xmax, ymin, xmax, ymax, xmin, ymax])
            self.__bbox_chars.append(char)

    # 最小外接矩形
    def __min_rectangle(self, xs, ys):
        xmin = np.min(xs)
        ymin = np.min(ys)
        xmax = np.max(xs)
        ymax = np.max(ys)
    ...
    def __get_proj_points(self, m, b, x0, y0):
        x1 = (m * y0 + x0 - m * b) / (m * m + 1)
        y1 = (m * m * y0 + m * x0 + b) / (m * m + 1)
        return x1, y1
    
    def __get_proj_len(self, k1, b1, x3, y3, x4, y4, y1, y2):
        x_proj_0, y_proj_0 = self.__get_proj_points(k1, b1, x3, y3)
        x_proj_1, y_proj_1 = self.__get_proj_points(k1, b1, x4, y4)
    ...
    def __compute_line_iou(self, x1, y1, x2, y2, x3, y3, x4, y4):
        ...
        ##计算点到直线的投影点
        dis1 = self.__get_proj_len(k1, b1, x3, y3, x4, y4, y1, y2)
        dis2 = self.__get_proj_len(k2, b2, x1, y1, x2, y2, y3, y4)
        l_len = min(l1_len, l2_len)
        iou_line = max(dis1 / (l_len + 0.0001), dis2 / (l1_len + 0.0001))
        return iou_line
    
    def __get_right_nearest_bbox(self, bboxes_x_center, bboxes_matrix, y_min_idx):
        ##左侧
        ...
        for i in range(bboxes_matrix_left.shape[0]):
            ...
            iou_line = self.__compute_line_iou(x1, y1, x4, y4, x2, y2, x3, y3)
    ...
    def __get_left_nearest_bbox(self, bboxes_x_center, bboxes_matrix, y_min_idx):
        ...
        # bbox_iou_line = np.zeros_like(bbox_y2, np.float32)
        ...
        # 如果当前未标点符号可能会出错
        # 判断iou_line 是否满足要求
        # iou_idx = np.where(bbox_iou_line * 1.0 < (1 * (cur_bbox_y3 - cur_bbox_y2) * (cur_bbox_y3 - cur_bbox_y2)))[0]
    ...
    def __line_post_process(self, line_res,bboxes_x_center,bboxes_y_center,bboxes_matrix):
        id_remove = []
        '''1.一个点，直接返回'''
        if len(line_res)==1:
    ...
    def get_results(self,):
        '''
        :param bboxes: 输入bboxes列表[x1, y1, x2, y2, x3, y3, x4, y4]
        :param im_data: 用于调试时数据可视化
        :return:
        '''
        if self.__bbox_locations.__len__() == 0:
            return []
        bboxes_matrix = np.array(self.__bbox_locations)  # [n,8]
        ...
        while (1):
            ...
            while (1):
                '''找到最左侧的bbox'''
                y_min_idx_left = self.__get_left_nearest_bbox(bboxes_x_center_in, bboxes_matrix_in, y_min_idx_in)
                # print("y_min_idx_left: ", y_min_idx_left)
                if y_min_idx_left != -1:
            ...
            '''stp 2.4 find right'''
            while (1):
                y_min_idx_right = self.__get_right_nearest_bbox(bboxes_x_center_in, bboxes_matrix_in, y_min_idx_in)
            ...
            line_res, id_remove = self.__line_post_process(line_res,bboxes_x_center,bboxes_y_center,bboxes_matrix)
            if len(id_remove)!=0:
